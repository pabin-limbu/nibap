          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
                className="h-9 px-3 text-[13px] tracking-tight font-medium border border-transparent data-[active=true]:border-border"
              >
                {item.url ? (
                  <Link href={item.url}>
                    <item.icon />
...
export function DashboardSidebar() {
  ...
  const othersMenuItems: MenuItem[] = [
    ...
    },
    {
      ...
      icon: Headphones,
      url: "mailto:changsu.pabin337@gmail.com",
    },
  ];
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <OrganizationSwitcher
                hidePersonal
                fallback={
                  <Skeleton className="h-8.5 group-data-[collapsible=icon]:size-8 rounded-md border border-border bg-white" />
                }
                appearance={{
                  elements: {
                    rootBox:
